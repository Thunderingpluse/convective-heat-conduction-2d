...
def solve_exp4b():
    print("\n2D Conduction Solver (Right Boundary Convection)")
    ...
    dx = L / n_div
    Bi_dx = (h * dx) / k # Biot-based factor [cite: 348]
    ...
    for i in range(n_nodes):
        for j in range(n_nodes):
            ...
            if i == 0: # Bottom
                A[idx, idx] = 1; B[idx] = t_bottom
            elif i == n_nodes - 1: # Top
                A[idx, idx] = 1; B[idx] = t_top
            elif j == 0: # Left
                A[idx, idx] = 1; B[idx] = t_left
            
            # Convective Boundary (Right Wall)
            elif j == n_nodes - 1:
                A[idx, idx] = -(4 + 2*Bi_dx)
                A[idx, get_idx(i, j-1)] = 2 # Ghost node substitution
                if i > 0: A[idx, get_idx(i-1, j)] = 1
                if i < n_nodes-1: A[idx, get_idx(i+1, j)] = 1
                B[idx] = -2 * Bi_dx * t_inf
            
            # Interior Nodes [cite: 211]
            else:
                A[idx, idx] = -4
    ...
    for i in range(n_nodes):
        for j in range(n_nodes):
            ...
            if i == 0:
                ...
                eq = f"-(4 + 2*Bi_dx)*T({j},{i}) + 2*T({j-1},{i})"
                if i > 0: eq += f" + T({j},{i-1})"
                if i < n_nodes-1: eq += f" + T({j},{i+1})"
                rhs = -2 * Bi_dx * t_inf
            else:
    ...
    plt.title(f'2D Convection Boundary (h={h} W/m²K)', weight='bold')
